...
from . import histogram, MutualInformation
import torch
...
def statistical_align_normal(source, target, patch_size, ch_sz=1, sig=2, normalize=True):
    MI = MutualInformation.MutualInformation(num_bins=patch_size ** 3, sigma=sig, normalize=normalize,
    ...
    if source.shape[1] == 1:
        ...
        for i in range(source.shape[1]):
            output[:, i:i + 1, :, :], output_sim[:, i:i + 1, :, :] = statistical_align(source[:, i, :, :].unsqueeze(1),
                                                                                       ...
                                                                                       patch_size, ch_sz, sig)
    return output, output_sim


# x0 = torch.randn(1, 1, 128, 128).cuda()
# _, sim_self = statistical_align(x0, x0, patch_size=3, sig=2)
# print(sim_self.size())
