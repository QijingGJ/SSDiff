...
class MutualInformation(nn.Module):
    ...
    def __init__(self, sigma=0.4, num_bins=256, device='cpu', normalize=True):
    ...
    def getMutualInformation(self, input1, input2):
        ...
        input1 = (input1 * 255).int()
        input2 = (input2 * 255).int()

        B, C, H, W = input1.shape
        assert ((input1.shape == input2.shape))

        x1 = input1.reshape(B, H * W, C)
        x2 = input2.reshape(B, H * W, C)
        ...
        if self.normalize:
            mutual_information = 2 * (mutual_information + self.epsilon) / (H_x1 + H_x2 + self.epsilon)
