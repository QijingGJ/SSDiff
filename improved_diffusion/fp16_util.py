...
def model_grads_to_master_grads(model_params, master_params):
    """
    Copy the gradients from the model parameters into the master parameters
    from make_master_params(). Handles parameters with None gradients.
    """
    grad_tensors = []
    ...
    for param in model_params:
        if param.grad is not None:
            ...
            # 无梯度：创建零梯度张量
            grad_tensors.append(torch.zeros_like(param.data).float())
...
def master_params_to_model_params(model_params, master_params):
    ...
    # 获取解压后的主参数
    unflattened = unflatten_master_params(model_params, master_params)

    # 更新模型参数
    for param, master_param in zip(model_params, unflattened):
        # 仅更新需要梯度的参数
